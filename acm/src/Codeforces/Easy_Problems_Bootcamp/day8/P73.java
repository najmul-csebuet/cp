...
public class P73 {
    ...
    private static void solution() {
        ...
        for (int w : map.keySet()) {
            list.add(new Freq(w, map.get(w)));
        }

        list.sort(Comparator.comparingInt(p -> p.item));
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            while (freq.count-- > 0) {
                if (!set.contains(freq.item - 1) && freq.item > 1) {
                    set.add(freq.item - 1);
                } else if (!set.contains(freq.item)) {
                    set.add(freq.item);
                } else if (!set.contains(freq.item + 1) && freq.item <= 150000) {
                    set.add(freq.item + 1);
                }
            }
        }
    ...
    private static class FastIO extends PrintWriter {
        ...
        public static FastIO getFastIO() throws FileNotFoundException {
            fileInOut = !P73.class.getPackage().getName().isEmpty();
