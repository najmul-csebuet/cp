...
public class CyclicPrimes {

    public static void main(String[] args) throws IOException {
        List<Boolean> l = new ArrayList<>();
        CyclicPrimes o = new CyclicPrimes();

        l.add(o.solution(1) == 0);
        l.add(o.solution(10) == 4);
        l.add(o.solution(100) == 13);
        l.add(o.solution(1000) == 25);
        l.add(o.solution(100000) == 43);
        l.add(o.solution(1000000) == 55);
    ...
    public int solution(int n) {
        ...
        ArrayList<Integer> primeList = AlgoHelper.getPrimesUpto(n);
        ...
        for (int i = 2; i <= n; i++) {
            ...
            for (int j = 1; j < prime.length(); j++) {
                String rotateRight = AlgoHelper.rotateRight(prime.toString(), 1);
                prime = new StringBuilder();
                prime.append(rotateRight);

                int parseInt = Integer.parseInt(rotateRight);
                if (parseInt >= primes.length) {
                    continue;
                }
                if (primes[parseInt]) {
                    flag = false;
    ...
    static class AlgoHelper {
        ...
        public static void printMissingNumber(int[] numbers, int count) {
            ...
            out.printf("Missing numbers in integer array %s, with total number %d is %n", Arrays.toString(numbers), count);
            int lastMissingIndex = 0;
            for (int i = 0; i < missingCount; i++) {
                lastMissingIndex = bitSet.nextClearBit(lastMissingIndex);
                out.println(++lastMissingIndex);
            }
        }
