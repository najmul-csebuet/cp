...
public class P38 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        boolean fileInOut = !P38.class.getPackage().getName().isEmpty();
        ...
        for (int t = 1; t <= totalTC; t++) {
            int N = io.nextInt();
            var S = io.next();
            new P38().solution(S.toCharArray());
        }
    ...
    public void solution(char[] chars) {
        for (int segmentSum = 0; segmentSum <= chars.length * 9; segmentSum++) {
            int segmentCount = 0;
            int currentSum = 0;
            for (int i = 0; i < chars.length; i++) {
                currentSum += (chars[i] - '0');
            ...
            if (segmentCount >= 2 && currentSum == 0) {
                io.println("YES");
                return;
            }
        }
