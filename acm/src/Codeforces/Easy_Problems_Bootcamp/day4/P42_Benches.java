...
public class P42_Benches {
    public static FastIO io;

    public static void main(String[] args) throws IOException {

        boolean fileInOut = !P42_Benches.class.getPackage().getName().isEmpty();
        ...
        for (int t = 1; t <= totalTC; t++) {
            ...
            int M = io.nextInt();
            var A = io.nextInts(N);
            new P42_Benches().solution(N, M, A);
        }
    ...
    public void solution(int n, int m, int[] a) {
        ...
        if (i > 0 || (i == 0 && t < 0)) {
            io.println(a[a.length - 1] + " " + (max + m));
            return;
        }

        if (t == 0) {
            io.println(max + " " + (max + m));
            return;
        }

        if (t > 0) {
            if (t % a.length == 0)
                io.println((max + t / a.length) + " " + (max + m));
            else
                io.println((max + t / a.length + 1) + " " + (max + m));
            return;
        }
    }
