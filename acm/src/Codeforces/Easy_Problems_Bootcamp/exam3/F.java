package Codeforces.Easy_Problems_Bootcamp.exam3;
...
public class F {
    ...
    private static void solution() {
        ...
        for (int i = 0; i < T.length - 1; i++) {
            ...
            int l = 1, r = 1, j = i + 1;
            while (j + 1 < T.length && T[j].equals(T[j + 1])) {
                ++j;
                ++r;
            }

            j = i;
            while (j - 1 >= 0 && T[j].equals(T[j - 1])) {
                --j;
                ++l;
            }
    ...
    private static class FastIO extends PrintWriter {
        ...
        public static FastIO getFastIO() throws FileNotFoundException {
            fileInOut = !F.class.getPackage().getName().isEmpty();
    ...
    private static class Util {
        ...
        static int[] suffixArray(String S) {
            ...
            class SuffixPair implements Comparable<SuffixPair> {
                ...
                public SuffixPair(int first, int second) {
                    this.first = first;
                    this.second = second;
                }
                ...
                public int compareTo(SuffixPair suffixPair) {
                    if (first != suffixPair.first) return first - suffixPair.first;
                    return second - suffixPair.second;
                }
            }

            class SuffixPair1 implements Comparable<SuffixPair1> {
                SuffixPair first;
                int second;

                public SuffixPair1(SuffixPair first, int second) {
                    this.first = first;
                    this.second = second;
                }

                @Override
                public int compareTo(SuffixPair1 suffixPair1) {
                    int i = first.compareTo(suffixPair1.first);
                    if (i != 0) {
                        return i;
                    }
                    return second - suffixPair1.second;
                }
            }
            ...
            while ((1 << k) < n) {
                // k -> k + 1
                var a = new SuffixPair1[n];
                for (int i = 0; i < n; i++) {
                    a[i] = new SuffixPair1(new SuffixPair(c[i], c[(i + (1 << k)) % n]), i);
                }
                Arrays.sort(a);
                for (int i = 0; i < n; i++) {
                    p[i] = a[i].second;
            ...
            for (int i = 0; i < n; i++) {
                io.println(p[i] + " " + S.substring(p[i], n - p[i]));
            }
