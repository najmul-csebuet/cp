...
public class P70 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        boolean fileInOut = !P70.class.getPackage().getName().isEmpty();
    ...
    public void solution() {
        var numberOfLocation = io.nextInt();
        var tomskPopulationCount = io.nextInt();
        List<City> cityList = new ArrayList<>();
        while (numberOfLocation-- > 0) {
            var X = io.nextInt();
            var Y = io.nextInt();
            var K = io.nextInt();
            var city = new City(X, Y, K);
            cityList.add(city);
        }
        cityList.sort(Comparator.comparingInt(p -> p.radius));

        int sum = tomskPopulationCount;
        for (int i = 0; i < cityList.size(); i++) {
            City city = cityList.get(i);
            if (sum + city.populationCount >= 1e6) {
                io.printf("%.7f\n", Math.sqrt(city.radius));
                return;
            }
            sum += city.populationCount;
        }
        io.println(-1);
    }

    static class City {
        Integer x, y, populationCount, radius;

        public City(Integer x, Integer y, Integer populationCount) {
            this.x = x;
            this.y = y;
