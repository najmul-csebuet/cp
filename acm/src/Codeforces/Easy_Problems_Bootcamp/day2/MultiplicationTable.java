...
public class MultiplicationTable {
    ...
    public static void main(String[] args) throws IOException {
        ...
        boolean fileInOut = !MultiplicationTable.class.getPackage().getName().isEmpty();
        ...
        for (int t = 1; t <= totalTC; t++) {
            int N = io.nextInt();
            int X = io.nextInt();
            new MultiplicationTable().solution(N, X);
        }
    ...
    public void solution(int n, int x) {

        int count = 0;
        int sqrt = (int) Math.sqrt(x);
        
        for (int i = 1; i <= sqrt; i++) {
            if (x % i == 0 && i <= n && x / i <= n) count += 2;
        }

        if (count > 0 && sqrt * sqrt == x) {
            --count;
        }
