package Codeforces.Easy_Problems_Bootcamp.exam2;
...
public class E {
    ...
    public void solution(int[] row1, int[] row2, int[] row3) {
        var square = new int[][]{row1, row2, row3};
        int a = square[0][2] + square[2][0];
        for (int i = 1; i < a; i++) {
            int r1Sum = i + row1[1] + row1[2];
            int r3Sum = row3[0] + row3[1] + a - i;
            if (r1Sum != r3Sum) continue;

            int diff = r1Sum - a;

            if (r1Sum != row2[0] + diff + row2[2]) continue;
