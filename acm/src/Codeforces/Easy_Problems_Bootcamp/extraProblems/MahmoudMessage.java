...
public class MahmoudMessage {
    ...
    private static void solution() {
        ...
        for (int i = 1; i <= N; i++) {
            var S = io.next();
            wordList.add(S);
            indexWordMap.put(i, S);
        }

        for (int i = 0; i < N; i++) {
            var wordCost = io.nextInt();
            wordCostMap.put(wordList.get(i), wordCost);
        }

        for (int i = 0; i < K; i++) {
            var groupIndexes = io.nextInts();
            int min = (int) (1e9 + 1);
            for (int g : groupIndexes) {
                String word = indexWordMap.get(g);
                Integer cost = wordCostMap.get(word);
                if (cost < min) {
                    min = cost;
                }
            }
            ...
            for (int g : groupIndexes) {
                String word = indexWordMap.get(g);
                wordCostMap.put(word, min);
            }
        }

        long sum = 0;
        while (M-- > 0) {
            sum += (wordCostMap.get(io.next()));
        }
    ...
    private static class FastIO extends PrintWriter {
        ...
        public static FastIO getFastIO() throws FileNotFoundException {
            fileInOut = !MahmoudMessage.class.getPackage().getName().isEmpty();
