...
public class P60 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        boolean fileInOut = false;//!P60.class.getPackage().getName().isEmpty();
    ...
    public void solution() {
        /*Integer[] arr = {1, 1, 2, 2, 2, 3, 4, 5, 5};
        int l = 0, r = arr.length - 1;
        io.println("Testing Lower Bound");
        io.print(arr);
        for (int x : Arrays.asList(1, 2, 3, 4, 5)) {
            io.println(x + "-->" + Util.lower_bound(arr, l, r, x));
        }

        io.println("Testing Upper Bound");
        io.print(arr);
        for (int x : Arrays.asList(1, 2, 3, 4, 5)) {
            io.println(x + "-->" + Util.upper_bound(arr, l, r, x));
        }*/
        int N = io.nextInt();
        int M = io.nextInt();
        var NA = io.nextInts(N);
        var MA = io.nextInts(M);
        Arrays.sort(NA);
        for (int i = 0; i < MA.length; i++) {
            int a = MA[i];
            int search = Util.upper_bound(NA, 0, NA.length - 1, a);//Arrays.binarySearch(NA, a);
            if (search >= 0) {
                io.print(search);
            }
