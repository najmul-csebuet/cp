...
public class P50 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        boolean fileInOut = !P50.class.getPackage().getName().isEmpty();
    ...
    public void solution(int x, int y, int n) {
        /*
            f[0] = x
            f[1] = y
            f[2] = y - x
            ...
            f[i] = f[i-1] - f[i-2] = f[i-2] - f[i-3] - f[i-2] = -f[i-3]
            f[i] = -f[i-3] = -(-f[i-6]) = -(-(-f[i-9])) = (-1)^x * f[i-3x]
         */
        var f = new int[]{x, y, y - x, -x, -y, x - y};
        var ans = f[(n - 1) % 6] % 1000000007;
