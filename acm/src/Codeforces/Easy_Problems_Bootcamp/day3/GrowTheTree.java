...
public class GrowTheTree {
    ...
    public static void main(String[] args) throws IOException {
        ...
        boolean fileInOut = !GrowTheTree.class.getPackage().getName().isEmpty();
        io = new FastIO(fileInOut);

        int totalTC = fileInOut ? io.nextInt() : 1;
        for (int t = 1; t <= totalTC; t++) {
            Integer[] A = io.nextInts();
            new GrowTheTree().solution(A);
        }
    ...
    public void solution(Integer[] a) {
        ...
        for (int i = a.length - 1; i >= mid; i--) {
            // use a[i] horizontally and then a[a.length - 1 - i]
            x += a[i];
            int j = a.length - 1 - i;
            if (i != j) {
                y += a[j];
