...
public class DeathNote {
    ...
    private static void solution() {
        ...
        long currentNameCount = 0;
        for (int i = 0; i < A.length; i++) {
            currentNameCount += A[i];
            if (currentNameCount < M) {
                if (i < A.length - 1) io.print("0 ");
                else io.println("0");
                continue;
            }
            long pageturned = currentNameCount / M;

            if (i < A.length - 1) io.print(pageturned + " ");
            else io.println(pageturned);
            currentNameCount = currentNameCount % M;
        }
        io.flush();
    }
    ...
    private static class FastIO extends PrintWriter {
        ...
        public static FastIO getFastIO() throws FileNotFoundException {
            fileInOut = !DeathNote.class.getPackage().getName().isEmpty();
