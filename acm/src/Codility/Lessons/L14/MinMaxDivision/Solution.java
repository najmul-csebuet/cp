package Codility.Lessons.L14.MinMaxDivision;
...
public class Solution {
    ...
    public int solution(int K, int M, int[] A) {
        ...
        maxBlockSize = (int) Math.ceil((float)N/K);
        normalBlockSize = N/K;

        int[] prefixSumArray = AlgoHelper.getPrefixSumArray(A);

        if (maxBlockSize == normalBlockSize) {
            int max = Integer.MIN_VALUE;
            for (int i = 0; i <= prefixSumArray.length - maxBlockSize; i+=normalBlockSize) {
                int segmentSum = prefixSumArray[i + maxBlockSize - 1] - prefixSumArray[i] + A[i];
                max = Math.max(max, segmentSum);
            }
