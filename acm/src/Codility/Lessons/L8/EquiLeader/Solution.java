package Codility.Lessons.L8.EquiLeader;
...
public class Solution {
    ...
    public int solution(int[] A) {
        ...
        int leaderIndex = AlgoHelper.getLeader(A);
        if (leaderIndex == -1) {
            out.println(0);
            return 0;
        }
        ...
        for (int i = 0; i < A.length; i++) {
            ...
            int prevValue = (i-1 >= 0) ? countLeader[i-1] : 0;

            if (A[i] == leader) {
                countLeader[i] = prevValue + 1;
            }
            else {
                countLeader[i] = prevValue;
            }
        }

        int ans = 0;
        for (int i = 0; i < countLeader.length; i++) {
            if ( ((countLeader[i] << 1) > (i+1)) && (((countLeader[countLeader.length - 1] - countLeader[i]) << 1) > (countLeader.length - i - 1)) ){
                ++ans;
            }
        }
