...
public class SegmentOccurrences {
    ...
    private static void solution() {
        ...
        while (Q-- > 0) {
            ...
            var R = io.nextInt();
            if (isIn[L] > 0)
                io.println(Math.max(prefix[R] - prefix[L] - isIn[L], 0));
            else
                io.println(prefix[R] - prefix[L]);
        }
    }
    ...
    private static class FastIO extends PrintWriter {
        ...
        public static FastIO getFastIO() throws FileNotFoundException {
            fileInOut = !SegmentOccurrences.class.getPackage().getName().isEmpty();
