...
public class F {
    ...
    public void solution(int a, int b) {
        var primeNumbers = Util.getPrimeNumbers((int) Math.sqrt(b));
        var isComposite = new boolean[(int) (b - a + 1)];
        for (int i = 0; i < primeNumbers.size(); i++) {
            ...
            if (a <= prime) {
                if (a == 1) isComposite[0] = true;
                s += prime;
            }

            for (int j = s; j < isComposite.length; j += prime) {
                isComposite[j] = true;
            }
        }
