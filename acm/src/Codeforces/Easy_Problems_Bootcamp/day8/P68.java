...
public class P68 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        boolean fileInOut = !P68.class.getPackage().getName().isEmpty();
    ...
    public void solution() {
        var totalLevel = io.nextInt();
        var X = io.nextInts();
        var Y = io.nextInts();

        var set = new HashSet<Integer>();
        for (int a : X) set.add(a);
        for (int a : Y) set.add(a);

        if (set.size() == totalLevel) {
            io.println("I become the guy.");
        } else {
            io.println("Oh, my keyboard!");
        }
    }
