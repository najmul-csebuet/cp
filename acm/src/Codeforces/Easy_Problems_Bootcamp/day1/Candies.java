...
public class Candies {
    ...
    public static void main(String[] args) throws IOException {
        ...
        boolean fileInOut = !Candies.class.getPackage().getName().isEmpty();
        io = new FastIO(fileInOut);

        int totalTC = fileInOut ? io.nextInt() : 1;
        for (int t = 1; t <= totalTC; t++) {
            int[] candies = new int[io.nextInt()];
            for (int i = 0; i < candies.length; ++i)candies[i] = io.nextInt();
            int A = io.nextInt();
            int B = io.nextInt();
            new Candies().solution(candies, A, B);
        }
    ...
    public void solution(int[] candies, int a, int b) {
        int ans = 0;
        for (int i = a; i <= b; i++) {
            ans += candies[i];
        }
