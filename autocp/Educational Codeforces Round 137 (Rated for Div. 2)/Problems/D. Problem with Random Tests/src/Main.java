...
public class Main {
    ...
    public void solution() {
        ...
        int i = S.indexOf('0');
        int lenRequired = S.length() - i;

        var best = "";

        char[] firstChars = S.toCharArray();

        char[] ansChars = new char[S.length()];
        for (int k = 0; k < S.length() - lenRequired; k++) {
            ansChars[k] = firstChars[k];
        }

        for (int j = 0; j <= S.length() - lenRequired; j++) {
            //var second = S.substring(j, j + lenRequired);
            for (int k = S.length() - lenRequired, m = 0; m < lenRequired && k < S.length(); k++, m++) {
                if (firstChars[j + m] == '1') {
                    ansChars[k] = '1';
                }
                else {
            ...
            String ans = String.valueOf(ansChars);
