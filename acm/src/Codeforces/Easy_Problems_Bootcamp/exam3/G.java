...
public class G {
    ...
    private static void solution() {
        var N = io.nextInt();
        var Q = io.nextInt();
        var Medals = io.next().toCharArray();
        var prefixSum = new MedalStat[N + 1];
        prefixSum[0] = new MedalStat(0, 0, 0);
        for (int i = 0; i < Medals.length; i++) {
            var p = new MedalStat(prefixSum[i], Medals[i]);
            prefixSum[i + 1] = p;
        }
        while (Q-- > 0) {
            var L = io.nextInt();
            var R = io.nextInt();
            var g = prefixSum[R].g - prefixSum[L - 1].g;
            var b = prefixSum[R].b - prefixSum[L - 1].b;
            var s = prefixSum[R].s - prefixSum[L - 1].s;
    ...
    static class MedalStat {
        int g, b, s;

        public MedalStat(int g, int b, int s) {
            this.g = g;
            this.b = b;
            this.s = s;
        }

        public MedalStat(MedalStat medalStat, char medal) {
            g = medalStat.g;
            b = medalStat.b;
            s = medalStat.s;
