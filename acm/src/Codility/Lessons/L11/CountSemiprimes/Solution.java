package Codility.Lessons.L11.CountSemiprimes;
...
public class Solution {
    ...
    public int[] solution(int N, int[] P, int[] Q) {
        ...
        ArrayList<Integer> primesUptoN = AlgoHelper.getPrimesUpto(N);
        Map<Integer, Boolean> map = new HashMap<>();
        //ArrayList<Integer> semiPrimes = new ArrayList<>();

        for (int i = 0; i < primesUptoN.size(); i++) {
            for (int j = 0; j < primesUptoN.size(); j++) {
                int semiPrime = primesUptoN.get(i) * primesUptoN.get(j);
                if (semiPrime > N)break;
                if (map.getOrDefault(semiPrime, false))continue;
                map.put(semiPrime, true);
                //semiPrimes.add(semiPrime);
            }
        }

        //Collections.sort(semiPrimes);
        //semiPrimes.sort(Comparator.naturalOrder());

        int[] prefixArray = new int[N+1];
        for (int i = 1; i <= N; i++) {
            if (map.getOrDefault(i, false)) {
                prefixArray[i] = (i-1 >= 0 ? prefixArray[i-1] : 0) + 1;
            }
            else {
                prefixArray[i] = i-1 >= 0 ? prefixArray[i-1] : 0;
            }
        }
        ...
        for (int i = 0; i < P.length; i++) {
            int a = P[i];
            int b = Q[i];
            ans[i] = prefixArray[b] - prefixArray[a - 1];
        }
