...
public class ScoresInOrder {
    ...
    public static void main(String[] args) throws IOException {
        ...
        boolean fileInOut = !ScoresInOrder.class.getPackage().getName().isEmpty();
        io = new FastIO(fileInOut);

        int totalTC = fileInOut ? io.nextInt() : 1;
        for (int t = 1; t <= totalTC; t++) {
            Integer[] S = io.nextInts();
            new ScoresInOrder().solution(S);
        }
    ...
    public void solution(Integer[] S) {

        for (int i = 0; i < S.length; i++) {
            // find rank of item i from item 0 to i
            int rank = 1;
            for (int j = 0; j < i; j++) {
                if (S[i] <= S[j]) ++rank;
            }
